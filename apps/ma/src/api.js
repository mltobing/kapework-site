 ...
 *   ma_posts.author_id          → ma_profiles.id
 *   ma_comments.author_id       → ma_profiles.id
 *   ma_attachments.post_id      → ma_posts.id
 *   ma_family_members.user_id   → ma_profiles.id
...
export async function fetchPosts(familyId, { limit = 20, offset = 0 } = {}) {
  const { data, error } = await supabase
    ...
    .select(`
      ...
      ma_attachments ( id, storage_path, mime_type )
    `)
 ...
 */
export async function createAttachment({ postId, storagePath, mimeType }) {
  const { data, error } = await supabase
    .from('ma_attachments')
    .insert({ post_id: postId, storage_path: storagePath, mime_type: mimeType })
    .select()
    .single();
  if (error) throw error;
  return data;
...
export async function fetchPeople(familyId) {
  const { data, error } = await supabase
    ...
    .select(`
      id, role, joined_at,
      ma_profiles!user_id ( id, display_name, relationship, avatar_url )
    `)
    .eq('family_id', familyId)
