...
const SEED_GRIDS = [
  ...
  // ── STARING (7) ───────────────────────────────────────────────────────────
  // Row0:G N I T  Row1:E R A S  Row2:L O D H  Row3:C B F W
  // STARING: S(1,3)→T(0,3)→A(1,2)→R(1,1)→I(0,2)→N(0,1)→G(0,0) ✓
  //   S(1,3)→T(0,3): Δrow=1 ✓  T(0,3)→A(1,2): Δrow=1,Δcol=1 ✓
  //   A(1,2)→R(1,1): Δcol=1 ✓  R(1,1)→I(0,2): Δrow=1,Δcol=1 ✓
  //   I(0,2)→N(0,1): Δcol=1 ✓  N(0,1)→G(0,0): Δcol=1 ✓
  // Also: RATING R(1,1)→A(1,2)→T(0,3)→I(0,2)→N(0,1)→G(0,0) ✓
  //   (same path reversed after the first two tiles)
  // Also: GRAIN G(0,0)→R(1,1)→A(1,2)→I(0,2)→N(0,1) ✓
  // Also: TINGE T(0,3)→I(0,2)→N(0,1)→G(0,0)→E(1,0) ✓
  'GNITERASLODHCBFW',

  // ── BREAKING (8) ──────────────────────────────────────────────────────────
  // Row0:S B R T  Row1:L E A K  Row2:O C I N  Row3:P D H G
  // BREAKING: B(0,1)→R(0,2)→E(1,1)→A(1,2)→K(1,3)→I(2,2)→N(2,3)→G(3,3) ✓
  //   B(0,1)→R(0,2): Δcol=1 ✓  R(0,2)→E(1,1): Δrow=1,Δcol=1 ✓
  //   E(1,1)→A(1,2): Δcol=1 ✓  A(1,2)→K(1,3): Δcol=1 ✓
  //   K(1,3)→I(2,2): Δrow=1,Δcol=1 ✓  I(2,2)→N(2,3): Δcol=1 ✓
  //   N(2,3)→G(3,3): Δrow=1 ✓
  // ING at (2,2),(2,3),(3,3) — distinct placement from the STIN/LOCG template
  'SBRTLEAKOCINPDHG',

  // ── SPEAKING (8) ──────────────────────────────────────────────────────────
  // Row0:T S P R  Row1:L E A K  Row2:O D I N  Row3:C F G W
  // SPEAKING: S(0,1)→P(0,2)→E(1,1)→A(1,2)→K(1,3)→I(2,2)→N(2,3)→G(3,2) ✓
  //   S(0,1)→P(0,2): Δcol=1 ✓  P(0,2)→E(1,1): Δrow=1,Δcol=1 ✓
  //   E(1,1)→A(1,2): Δcol=1 ✓  A(1,2)→K(1,3): Δcol=1 ✓
  //   K(1,3)→I(2,2): Δrow=1,Δcol=1 ✓  I(2,2)→N(2,3): Δcol=1 ✓
  //   N(2,3)→G(3,2): Δrow=1,Δcol=1 ✓
  // ING at (2,2),(2,3),(3,2) — distinct placement
  'TSPRLEAKODINCFGW',

  // ── THROWING (8) ──────────────────────────────────────────────────────────
  // Row0:T H R A  Row1:S E O W  Row2:L C I N  Row3:P D F G
  // THROWING: T(0,0)→H(0,1)→R(0,2)→O(1,2)→W(1,3)→I(2,2)→N(2,3)→G(3,3) ✓
  //   T(0,0)→H(0,1): Δcol=1 ✓  H(0,1)→R(0,2): Δcol=1 ✓
  //   R(0,2)→O(1,2): Δrow=1 ✓  O(1,2)→W(1,3): Δcol=1 ✓
  //   W(1,3)→I(2,2): Δrow=1,Δcol=1 ✓  I(2,2)→N(2,3): Δcol=1 ✓
  //   N(2,3)→G(3,3): Δrow=1 ✓
  // ING at (2,2),(2,3),(3,3) — top-left orientation of featured word
  'THRASEOWLCINPDFG',

  // ── FLOATING (8) ──────────────────────────────────────────────────────────
  // Row0:R S H G  Row1:C B N I  Row2:D E T A  Row3:F L O P
  // FLOATING: F(3,0)→L(3,1)→O(3,2)→A(2,3)→T(2,2)→I(1,3)→N(1,2)→G(0,3) ✓
  //   F(3,0)→L(3,1): Δcol=1 ✓  L(3,1)→O(3,2): Δcol=1 ✓
  //   O(3,2)→A(2,3): Δrow=1,Δcol=1 ✓  A(2,3)→T(2,2): Δcol=1 ✓
  //   T(2,2)→I(1,3): Δrow=1,Δcol=1 ✓  I(1,3)→N(1,2): Δcol=1 ✓
  //   N(1,2)→G(0,3): Δrow=1,Δcol=1 ✓
  // ING at (1,3),(1,2),(0,3) — featured word runs bottom→top for novelty
  'RSHGCBNIDETAFLOP',
];
