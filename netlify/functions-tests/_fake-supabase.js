 ...
 * exercise the trusted-device / activity / document-inbox Netlify Functions
 * end-to-end without a network call. Kept OUT of netlify/functions/
 * (test-only, never shipped — see _ma-crypto.test.js for why that split
 * matters for Netlify's bundler).
 *
 ...
 *
 * A fixture may optionally provide `storage`, a map of
 * `{ [bucketName]: async (path) => ({ data, error }) }`, to back
 * `supabase.storage.from(bucket).download(path)` calls (used by the Document
 * Inbox worker — see _ma-document-processing.test.js).
 */
  ...
  return {
    ...
    storage: {
      from(bucket) {
        return {
          download: async (path) => {
            const fn = currentFixture.storage && currentFixture.storage[bucket];
            if (!fn) return { data: null, error: { message: `no storage fixture for bucket "${bucket}"` } };
            return fn(path);
          },
        };
      },
